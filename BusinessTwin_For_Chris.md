# Business Twin — code handoff (Chris)

**Live demo:** https://business-twin.vercel.app

Mock-data GED 2026 prototype. The entire product UI lives in **one file**: `src/App.jsx`. Everything else is stock Vite + React scaffold.
...
```bash
npm create vite@latest business-twin -- --template react
cd business-twin
npm install
npm install recharts
```

Then replace `src/App.jsx` with the code below, and **empty** `src/App.css` and `src/index.css`.
...
| Query chip | What it shows |
|------------|----------------|
| Hire 2 people | Default high-confidence yes (+14%, 1 in 4 crunch) |
| Hire 1 dispatcher | Lighter cohort outcome (1 hire profile) |
| Hire 3 technicians | Heavier crunch tail (3 hire profile) |
| Refrigeration specialist | Thin cohort → low confidence |
| Second location | Honest **no** — negative verdict |

Also try: **How this works** panel, **movers ▾** drill-down sliders, **stagger hire** slider (section 03).
...
|---------------------|---------|
| `BUSINESS` | Mock ledger (Riverside HVAC) |
| `POOL` / `POOL_THIN` / `POOL_EXPANSION` | Locked cohort denominators |
| `OUTCOMES_BY_HIRES` | Histogram + median + crunch % per hire count |
| `resolveBaseCohort()` | Query → cohort (hiring / niche / expansion) |
| `applyDrillFilters()` | Cohort drill-down sensitivity |
| `buildCashSeries()` | Ledger projection + stagger slider |
| `computeUnderwriting()` | Why $40K not $80K capital offer |

---

## `src/App.jsx` (full source)
...
*Prototype · mock data · Brice Hatcher · May 2026*
